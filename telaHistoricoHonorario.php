...
<div class="content-wrapper">
  ...
  <div class="container">
    <?php 
      ...
      $queryGetRegistros = "select * from controle_de_honorarios_esp ";
      //$queryGetRegistros = "select * from historico";
      $resultCount = mysqli_query($conn,"select count(*) as total_records from controle_de_honorarios_esp");
      ...
      mysqli_query($conn,"select * from controle_de_honorarios_esp where $tipoFiltro 
      like '%$valorFiltro%' LIMIT $offset,$total_records_per_page"):
      //Query para caso não for preenchido filtro
      mysqli_query($conn,"select * from controle_de_honorarios_esp LIMIT $offset,
...
?>
  ...
  <?php 
    ...
    //  echo "<td><a href='telaAlteracao.php?id=".$row["numeroProcesso"]."'><button type='button' class='btn btn-sm btn-primary mr-1' data-toggle='modal' data-target='#modalAlteracao'><i class='fas fa-pencil-alt'></i> Editar</button></a>";
      echo "<td><a href='telaRecuperacao.php?id=".$row["numeroProcesso"]."'><button type='button' class='btn btn-sm btn-secondary mr-1'><i class='fas fa-sync'></i> Recuperar</button></a></td>";
    //  echo"</td>";
      echo "</tr>";
